...
<body id="page-top">
    <!-- Navigation-->
    <nav class="navbar navbar-expand-lg navbar-dark zdn-bg fixed-top" id="sideNav">
        <a class="navbar-brand js-scroll-trigger" href="#page-top">
            <span class="d-block d-lg-none">{{ $about->judul }}</span>
            <span class="d-none d-lg-block"><img class="img-fluid img-profile rounded-circle mx-auto mb-2"
                    src="{{ asset('foto') . '/' . get_meta_data('_foto') }}" alt="..." /></span>
        </a>
        ...
        <section class="resume-section" id="about">
            <div class="resume-section-content">
                ...
                <div class="subheading mb-5">
                    {{ get_meta_data('_kota') }} · {{ get_meta_data('_provinsi') }} · {{ get_meta_data('_nohp') }} ·
                    <a href="mailto:name@email.com" class="zdn-text">{{ get_meta_data('_email') }}</a>
                </div>
                <p class="lead mb-5">{!! $about->isi !!}</p>
                <div class="social-icons">
                    <a class="social-icon" href="{{ get_meta_data('_linkedln') }}"><i
                            class="fab fa-linkedin-in"></i></a>
                    <a class="social-icon" href="{{ get_meta_data('_github') }}"><i class="fab fa-github"></i></a>
                    <a class="social-icon" href="{{ get_meta_data('_instagram') }}"><i
                            class="fab fa-instagram"></i></a>
                    <a class="social-icon" href="{{ get_meta_data('_facebook') }}"><i
        ...
        <section class="resume-section" id="skills">
            <div class="resume-section-content">
                ...
                <ul class="list-inline dev-icons">
                    @foreach (explode(', ', get_meta_data('_language')) as $item)
                        <li class="list-inline-item"><i class="devicon-{{ strtolower($item) }}-plain"></i></li>
                    @endforeach
                </ul>
                <div class="subheading mb-3">Workflow</div>
                {!! set_list_workflow(get_meta_data('_workflow')) !!}
            </div>
        </section>
        <hr class="m-0" />
        <!-- Interests-->
